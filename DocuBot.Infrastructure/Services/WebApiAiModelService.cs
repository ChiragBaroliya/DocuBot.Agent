...
namespace DocuBot.Infrastructure.Services
{
    public class WebApiAiModelService : IAiModelService
    {
        ...
        {
            var payload = new { ProjectDescription = projectDescription };
            var response = await _httpClient.PostAsJsonAsync($"{_baseUrl}/api/commit/master-functional-readme", payload);
            response.EnsureSuccessStatusCode();
            return await response.Content.ReadAsStringAsync();
        }
    }
